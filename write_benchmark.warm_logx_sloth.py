...
mtx_coo_noz = read_and_clean_benchmark_data('sloth/warm_write/br_mtx_noz.out')
binsparse_coo_noz = read_and_clean_benchmark_data('sloth/warm_write/br_coo_noz.out')
binsparse_coo_gzip1 = read_and_clean_benchmark_data('sloth/warm_write/br_coo_gz1.out')
binsparse_csr_noz = read_and_clean_benchmark_data('sloth/warm_write/br_csr_noz.out')
binsparse_csr_gzip1 = read_and_clean_benchmark_data('sloth/warm_write/br_csr_gz1.out')
...
datasets = [mtx_coo_noz, binsparse_coo_gzip1, binsparse_csr_gzip1, binsparse_coo_noz, binsparse_csr_noz]
labels = ['mtx_noz_noaux', 'binsparse_coo_gzip1', 'binsparse_csr_gzip1', 'binsparse_coo_noz', 'binsparse_csr_noz']

colors = ['C0', 'C3', 'C4', 'C1', 'C2']

ytick_data = [0.006, 0.03, 0.18, 1, 6, 30, 180]
ytick_data = [0.01, 0.01, 0.1, 1, 10, 100]
...
plot_sizes_logx(mtx_noz_noaux, datasets, labels, ordering, title='Write Times (Unflushed)', y_title='Runtime', x_title='Matrix Market File Size', yticks = (ytick_data, ytick_labels), xticks = (xtick_data, xtick_labels), colors=colors, fname='out.pdf')

print_speedups(datasets, labels, ordering, mtx_coo_noz)

print_statistics(['sloth/warm_write/br_coo_gz1.out', 'sloth/warm_write/br_coo_noz.out', 'sloth/warm_write/br_csr_gz1.out', 'sloth/warm_write/br_csr_noz.out', 'sloth/warm_write/br_mtx_noz.out'], ordering)
